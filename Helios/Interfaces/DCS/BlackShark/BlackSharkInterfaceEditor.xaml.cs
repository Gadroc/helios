...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.BlackShark
{
    ...
    /// </summary>
    public partial class BlackSharkInterfaceEditor : HeliosInterfaceEditor
    {
        static BlackSharkInterfaceEditor()
        {
            Type ownerType = typeof(BlackSharkInterfaceEditor);
        ...
        {
            BlackSharkInterfaceEditor editor = target as BlackSharkInterfaceEditor;
        ...
        public BlackSharkInterfaceEditor()
        {
            InitializeComponent();

            Configuration = new DCSConfigurator("DCSBS", DCSPath);
            Configuration.ExportConfigPath = "Config\\Export";
            Configuration.ExportFunctionsPath = "pack://application:,,,/Helios;component/Interfaces/DCS/BlackShark/ExportFunctions.lua";
        }
        ...
        public static readonly DependencyProperty ConfigurationProperty =
            DependencyProperty.Register("Configuration", typeof(DCSConfigurator), typeof(BlackSharkInterfaceEditor), new PropertyMetadata(null));
        ...
        public string DCSPath
        {
            get
            {
                if (_dcsPath == null)
                {
                    ...
                    if (pathKey == null)
                    {
                        pathKey = Registry.CurrentUser.OpenSubKey(@"Software\Eagle Dynamics\DCS KA-50");
                    }
                    if (pathKey == null)
                    {
                    ...
                    if (pathKey != null)
                    {
                        ...
                        pathKey.Close();
                        ConfigManager.LogManager.LogDebug("DCS Black Shark Interface Editor - Found DCS Path (Path=\"" + _dcsPath + "\")");
                    }
                    else
                    {
                        _dcsPath = "";
        ...
        private void Configure_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show(Window.GetWindow(this), "DCS Black Shark has been configured.");
            }
            else
            {
                MessageBox.Show(Window.GetWindow(this), "Error updating DCS Black Shark configuration.  Please do one of the following and try again:\n\nOption 1) Run Helios as Administrator\nOption 2) Install DCS outside the Program Files Directory\nOption 3) Disable UAC.");
            }
        }
