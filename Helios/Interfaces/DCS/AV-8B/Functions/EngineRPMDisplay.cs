...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.AV8B.Functions
{
    ...
    public class EngineRPMDisplay : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[] { new DCSDataElement("2002", null, false) };

        private HeliosValue _four_digit_display;

        public EngineRPMDisplay(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            _four_digit_display = new HeliosValue(sourceInterface, BindingValue.Empty, "EDP", "RPM display", "Engine RPM percentage", "", BindingValueUnits.Numeric);
