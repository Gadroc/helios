...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.AV8B.Functions
{
    ...
    public class FuelBingoDisplay : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[] { new DCSDataElement("2013", null, false) };

        private HeliosValue _four_digit_display;

        public FuelBingoDisplay(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            _four_digit_display = new HeliosValue(sourceInterface, BindingValue.Empty, "FQIS", "Bingo display", "Fuel Bingo value", "", BindingValueUnits.Numeric);
