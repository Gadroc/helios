...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.AV8B.Functions
{
    ...
    public class EngineFFDisplay : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[] { new DCSDataElement("2003", null, false) };

        private HeliosValue _three_digit_display;

        public EngineFFDisplay(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            _three_digit_display = new HeliosValue(sourceInterface, BindingValue.Empty, "EDP", "FF display", "Engine FF percentage", "", BindingValueUnits.Numeric);
