...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.AV8B.Functions
{
    ...
    public class EngineDuctDisplay : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[] { new DCSDataElement("2001", null, false) };

        private HeliosValue _three_digit_display;

        public EngineDuctDisplay(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            _three_digit_display = new HeliosValue(sourceInterface, BindingValue.Empty, "EDP", "Duct pressure display", "Engine Duct pressure", "", BindingValueUnits.Numeric);
