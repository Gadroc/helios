...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.AV8B.Functions
{
    ...
    public class FuelLeftDisplay : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[] { new DCSDataElement("2011", null, false) };

        private HeliosValue _four_digit_display;

        public FuelLeftDisplay(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            _four_digit_display = new HeliosValue(sourceInterface, BindingValue.Empty, "FQIS", "Left Tank display", "Fuel left tank quantity", "", BindingValueUnits.Numeric);
