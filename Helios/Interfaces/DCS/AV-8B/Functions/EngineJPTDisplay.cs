...
namespace GadrocsWorkshop.Helios.Interfaces.DCS.AV8B.Functions
{
    ...
    public class EngineJPTDisplay : NetworkFunction
    {
        private static DCSDataElement[] _dataElements = new DCSDataElement[] { new DCSDataElement("2004", null, false) };

        private HeliosValue _three_digit_display;

        public EngineJPTDisplay(BaseUDPInterface sourceInterface)
            : base(sourceInterface)
        {
            _three_digit_display = new HeliosValue(sourceInterface, BindingValue.Empty, "EDP", "JPT display", "Engine Jet pipe temperature", "", BindingValueUnits.Numeric);
